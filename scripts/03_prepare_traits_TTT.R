#####################################################################
# Preprocess the Tundra Trait Team (TTT) datasets

library(tidyverse)
library(data.table)

df <- read_csv("/scratch/project_2003061/trait_datasets/TTT/TTT_cleaned_dataset_v1.csv") %>% as.data.table()

df <- df %>% select(AccSpeciesName, Latitude, Longitude, Trait, Value, Units, DataContributor)
...
refs <- df2 %>% select(DataContributor) %>% 
  group_by(DataContributor) %>% count() %>% 
  ungroup() %>% 
  mutate(TTT_source = paste0("TTT_", rownames(.)))

df2 <- df2 %>% full_join(., refs %>% select(-n))

df2 %>% select(-AccSpeciesName, -DataContributor) %>% write_csv("/scratch/project_2003061/trait_datasets/TTT/traits_prepared.csv")
